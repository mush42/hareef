...
from hareef.utils import format_as_table
...
class DatasetUtils:
    ...
    def __init__(self, config):
        ...
        self.delimiters = config["sentence-break"]["delimiters"]

        self.numbers = [c for c in "0123456789"]
        self.letter_list = self.special_tokens + list(constants.ARABIC_LETTERS_LIST)
        self.diacritic_list = [' '] + list(constants.DIACRITICS_LIST)

        self.debug = config["debug"]

        self.stride = config["segment"]["train"]["stride"]  
        self.window = config["segment"]["train"]["window"]  

        self.test_stride = config["segment"]["test"]["stride"]  
        self.test_window = config["segment"]["test"]["window"] 
    ...
    def load_mapping_v3(self, dtype, file_ext=None):
        ...
        f_name = os.path.join(self.base_path, dtype + file_ext)
    ...
    def load_embeddings(self, embs_path, limit=-1):
        ...
        _LOGGER.info(f"[INFO] Reading Embeddings from {embs_path}")
    ...
    def load_file_clean(self, dtype, strip=False):
        f_name = os.path.join(self.base_path, "dataset", dtype + ".txt")
    ...
    def stats(self, freq, percentile=90, name="stats"):
        ...
        cols = list(zip(
            ["Dataset", "Mean", "Std", "Min", "Max", f"{percentile}th Percentile"],
            [name, freq.mean(), freq.std(), freq.min(), freq.max(), np.percentile(freq, percentile)]
        ))
        table = format_as_table(*cols)
