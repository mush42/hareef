...
class BidirectionalRecurrentLayer(nn.Module):
    ...
    def forward(self, input, state_t0, is_last):
        ...
        if return_states:
            ...
            return T.cat([fwd, bwd], dim=-1), (
                T.cat([state_fwd[0], state_bwd[0]], dim=-1),
                T.cat([state_fwd[1], state_bwd[1]], dim=-1),
            )
        else:
        ...
        **kargs,
        ...
        if bidirectional:
            ...
            def make(isize: int, last=False):
                ...
                return RecurrentLayer(cell, isize, batch_first=batch_first)

        if num_layers > 1:
            rnns = [
                make(input_size),
                *[make(Dh) for _ in range(num_layers - 2)],
                make(Dh, last=True),
            ]
        else:
